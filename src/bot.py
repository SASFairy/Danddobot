    ...
    async def on_message(self, message: discord.Message):
        ...
        async with self.lock:
            ...
            block_instructions = (
                ...
                "차단을 내리기로 결정했다면, 답변 글의 맨 첫 줄에 반드시 `[BLOCK_USER]` 태그를 정확하게 삽입하고, 그 뒤에는 진짜 급정색하며 거친 쌍욕과 함께 차가운 한 줄짜리 한마디 멘트를 쏘아붙이십시오.\n"
                "반드시 차단 결정 시 맨 첫 줄에 `[BLOCK_USER]` 태그를 넣고, 뒷부분에는 가식 없는 급정색 말투로 짧은 욕설을 단호하고 단순하게 적으십시오. 시스템적인 카드나 안내 가이드는 적지 마십시오. 예시 문장은 따로 드리지 않으니 단또봇의 콘셉트(야옹체)에 어울리는 정색 욕설 대사를 다양하고 매우 창의적으로 직접 작문하여 출력하십시오."
            )
